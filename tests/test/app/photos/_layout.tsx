...
export default function PhotosLayout() {
  return (
    <YStack flex={1} bg="$background" data-testid="photos-layout">
      <XStack p="$3" gap="$3" ai="center" bg="$color2">
        <Link href="/">
          <Text color="$blue10" fontSize="$4">
            Home
          </Text>
        </Link>
        <Text fontSize="$5" fontWeight="bold" data-testid="photos-title">
          Photos
        </Text>
      </XStack>

      <ScrollView flex={1}>
        <XStack flexWrap="wrap" p="$2" gap="$3" jc="center">
          {photos.map((photo) => (
