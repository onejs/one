...
test(
  'navigate into nested page, then back and forward to same page',
  ...
  async () => {
    ...
    await page.waitForSelector('[data-testid="navigate-into-nested-page"]', {
      timeout: 60_000,
      state: 'visible',
    })
    ...
    expect(
      (await page.getByTestId('pathname').textContent())?.endsWith(
