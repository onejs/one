...
test('import.meta.env', sharedTestOptions, async () => {
  ...
  const testEnvValue = await driver
    .$('~import-meta-env-VITE_TEST_ENV_VAR_1-value')
    .getText()
  expect(testEnvValue).toBe('test_value_1')

  const testEnvValue2 = await driver
    .$('~import-meta-env-VITE_TEST_ENV_VAR_1-value-2')
    .getText()
  expect(testEnvValue2).toBe('test_value_1')

  const testModeSpecificEnvValue = await driver
