...
describe('CSS Hydration Tests', () => {
  ...
  it('should have matching CSS element count between server HTML and hydrated DOM', async () => {
    ...
    const serverLinkStylesheetCount = (html.match(/<link[^>]*rel=["']stylesheet["'][^>]*>/g) || [])
      .length
  ...
  it('should preserve inline CSS content after full hydration', async () => {
    ...
    const clientCSSContents = await page.evaluate(() => {
      const styles = document.querySelectorAll<HTMLStyleElement>('style[id^="__one_css_"]')
      const result: Record<string, { hasContent: boolean; contentLength: number; content: string }> =
        {}
