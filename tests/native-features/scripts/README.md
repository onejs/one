...
`web-browser` covers `One.WebBrowser`: a user Done-tap resolves cancel, a programmatic dismiss resolves dismiss on both the open and dismiss promises, and the auth session resolves cancel from whichever Cancel prompt is showing. The redirect success path needs a live identity provider, so the suite covers cancel only.
