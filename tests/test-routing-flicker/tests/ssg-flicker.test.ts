...
const SSG_ROUTES_WITH_LOADERS = [
  ...
  { path: '/docs/api-reference', expectedText: 'API Reference', contentId: 'doc-title' },
  {
    ...
    expectedText: 'Advanced Usage',
    contentId: 'doc-title',
  },
]

// SSG route without loader
const SSG_ROUTE_NO_LOADER = {
  path: '/no-loader',
  expectedText: 'No Loader Page',
  contentId: 'no-loader-title',
}
...
const DEFAULT_MODE_ROUTES = [
  {
    path: '/default-mode',
    expectedText: 'Default Mode Index',
    contentId: 'default-title',
  },
  {
    path: '/default-mode/page-one',
    expectedText: 'Page One',
    contentId: 'default-slug-title',
  },
  {
    path: '/default-mode/page-two',
    expectedText: 'Page Two',
    contentId: 'default-slug-title',
  },
  {
    path: '/default-mode/page-three',
    expectedText: 'Page Three',
    contentId: 'default-slug-title',
  },
]
  ...
  await page.evaluate((minLen) => {
    ...
    const checkFlicker = () => {
      ...
      if (container && container.textContent) {
        ...
        ;(window as any).__minContentBeforeFlicker = Math.min(
          (window as any).__minContentBeforeFlicker,
          len
        )
...
describe('SSG Hydration - No Flicker on Initial Load', () => {
  for (const route of SSG_ROUTES_WITH_LOADERS) {
    test(`${route.path} has content immediately at DOM ready`, async () => {
      ...
      expect(
        hasExpectedContent,
        `Expected "${route.expectedText}" in HTML at DOM ready for ${route.path}`
      ).toBe(true)
      expect(hasAppContainer, `Expected app-container in HTML for ${route.path}`).toBe(
        true
      )
      expect(
        hasSubstantialContent,
        `Expected substantial HTML content for ${route.path}`
      ).toBe(true)
      expect(
        contentText,
        `Expected #${route.contentId} to exist for ${route.path}`
  ...
  test('SSG page without loader has content at DOM ready', async () => {
    ...
    await page.goto(serverUrl + SSG_ROUTE_NO_LOADER.path, {
...
describe('SSG Navigation - Dynamic Route to Dynamic Route', () => {
  test('/docs/getting-started => /docs/api-reference - no flicker', async () => {
    ...
    expect(
      ...
      `Flicker detected! Min content: ${results.minContentLength}`
    ).toBe(false)
  ...
  test('/docs/getting-started => /docs/api-reference => /docs/advanced-usage - chain navigation no flicker', async () => {
    ...
    expect(
      results.flickerDetected,
      `Flicker during chain navigation! Details: ${JSON.stringify(results.flickerDetails)}`
    ).toBe(false)
...
describe('Cross-Mode Navigation - SSG to Default Mode', () => {
  ...
  test.skip('/ => /default-mode => /docs => / - full circuit no flicker', async () => {
    ...
    expect(
      results.flickerDetected,
      `Flicker in full circuit! Details: ${JSON.stringify(results.flickerDetails)}`
    ).toBe(false)
...
describe('Browser History Navigation (Back/Forward)', () => {
  ...
  test.skip('back/forward between SSG dynamic routes preserves content without flicker', async () => {
    ...
    expect(
      results.flickerDetected,
      `Flicker during back/forward! Details: ${JSON.stringify(results.flickerDetails)}`
    ).toBe(false)
...
describe('SSG Content Without JavaScript', () => {
  for (const route of [...SSG_ROUTES_WITH_LOADERS, SSG_ROUTE_NO_LOADER]) {
    test(`${route.path} has full content with JS disabled`, async () => {
      ...
      expect(
        hasExpectedContent,
        `SSG page ${route.path} should have "${route.expectedText}" without JS`
      ).toBe(true)
      expect(
        hasAppContainer,
        `SSG page ${route.path} should have app-container without JS`
      ).toBe(true)
...
describe('Hydration Console Errors', () => {
  const routesToCheck = ['/docs/getting-started', '/docs/api-reference', '/', '/docs']
...
describe.skip('Rapid Navigation Stress Test', () => {
  test('rapid clicks between SSG pages does not cause flicker', async () => {
    ...
    expect(
      results.flickerDetected,
      `Flicker during rapid nav! Min content: ${results.minContentLength}`
    ).toBe(false)
