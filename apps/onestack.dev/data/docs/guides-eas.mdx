...
2. Add a `react-native.config.cjs` to your project root (next to your `package.json`) with the following content:
   ...
   module.exports = {
     ...
     // where Vite shall be used instead of Metro for JS bundling during the build process.
     commands: [...require('one/react-native-commands')],
   }
   ```

3. Modify your `app.json` (or `app.config.js`) and add `one/expo-plugin` to `expo.plugins`. This plugin will hook into the Expo build process running on EAS to ensure your One app is built correctly:
...
4. Add a `postinstall` script to your `package.json` that runs `one patch`. This applies necessary patches to Expo's CLI to ensure compatibility with One's build process:
   ...
   One attempts to patch this but there can be timing issues, so this is a safer setup.
