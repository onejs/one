...
Metro mode runs no Babel. One replaces Expo's Babel transformer with its own worker built on oxc, so `babel.config.js` and `babel-preset-expo` have no effect and every plugin One used to ship on native is a Rust port instead.
...
A native transform is the shape a Babel plugin takes when there is no Babel — source in, source out, free to use oxc, a regex, or anything else:
...
Return `null` to leave the file alone. Transforms run in order, before One's own ports, on the file exactly as it came off disk — so a transform that reads positions sees the original line numbers, the same view a Babel plugin got.
