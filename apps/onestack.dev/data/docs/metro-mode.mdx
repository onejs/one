...
- Set `native.bundlerOptions.nativeTransforms: false` to use Babel, or `ONE_METRO_NATIVE_TRANSFORMS=0` for a single run. Install `@react-native/babel-preset`, then run `one metro-eject` to create a Babel config that composes React Native's preset with `one/babel-preset`. Expo presets are optional app configuration and are never loaded by One.
