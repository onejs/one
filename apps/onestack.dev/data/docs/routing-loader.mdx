...
```tsx fileName=app/cards/[id]+ssr.tsx
import { useLoader } from 'one'
import { db } from '~/lib/db'

// concurrent requests with the same id share one loader call
export const loaderCache = (params: { id: string }) => params.id
...
```tsx
export const loaderCache = (params: { id: string }) => ({
  key: params.id,
...
```tsx
export const loaderCache = (params, request) => {
  const userId = request?.headers.get('x-user-id') || 'anon'
  return `${params.id}:${userId}`
}
```

### When to use loaderCache

- **Expensive queries under load** — routes with heavy database queries that get concurrent traffic (popular pages, viral content)
- **Identical results across users** — routes where the loader result doesn't depend on auth. For auth-dependent loaders, include user identity in the key
- **High concurrency** — in benchmarks with 50 concurrent connections to the same route, `loaderCache` improved throughput from ~570 req/sec to ~2,000 req/sec (3.5x)
