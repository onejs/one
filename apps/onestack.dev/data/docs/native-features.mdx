...
Names are Material Symbols snake_case names from fonts.google.com/icons, such as `star` or `arrow_forward`. There is no mapping to SF Symbols: iOS keeps `systemImage`/`systemName` and web renders nothing.

`size` defaults to 24. `filled` picks the bundled filled font instead of outlined. `Compose.Button` takes a leading icon with `icon` and `iconFilled`. Tint with `composeStyle.foregroundColor`. Fonts are bundled, so rendering is offline and synchronous. Unknown names throw at mount.
