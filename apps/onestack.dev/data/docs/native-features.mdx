...
For routed screens, prefer [`Stack.Toolbar`](/docs/components-Stack#stacktoolbar). It adds header and bottom toolbars with buttons, menus, spacers, custom views, and search-bar placement. The primitives below are the lower-level API used by `Stack.Toolbar`.
