...
<Notice>
  Render `<Redirect>` declaratively from your guard. Don't replace it with a manual `useEffect` + `useRef` + `router.replace`. While auth state settles during a sign-in, the guard re-renders rapidly and may remount `<Redirect>`, but `<Redirect>` de-duplicates the navigation across those remounts. A hand-rolled effect that fires `router.replace` on every render can thrash the navigator into `Maximum update depth exceeded`.
</Notice>
