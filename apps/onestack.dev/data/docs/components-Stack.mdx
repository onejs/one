...
Native toolbar items use the same compound style as the header. `Stack.Toolbar`
with `Leading`/`Trailing` slots (`Left`/`Right` are aliases) compiles to
native header items on iOS, and `Stack.Toolbar.Bottom` renders a native
bottom toolbar from screen content:
...
See [native features](/docs/native-features) for the full toolbar reference,
including menus, the bottom toolbar, and the lower-level `One.iOS.ToolbarHost`
primitive.
