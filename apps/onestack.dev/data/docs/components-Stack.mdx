...
On web, `<Stack>` renders headless: only the focused screen's element, no header, no chrome. Screens with a `presentation` (`sheet`, `modal`, `formSheet`, etc.) render as overlays above the base screen - by default inline with no floating chrome, or with real chrome once you mount [Presentations](/docs/components-Presentations).
...
See the [headless web navigators guide](/docs/guides-headless-web) for the full story, including how `Presentations` maps sheets and modals to real chrome.
