...
with `placement="left"` or `placement="right"` compiles to native header
items on iOS, and `placement="bottom"` (the default) renders a native bottom
toolbar from screen content:
