...
One comes with a serve command that is powered by [Hono](https://hono.dev). For high-traffic production deployments, use `--cluster` to fork multiple worker processes that share the same port. See [one serve](/docs/one-serve) for details.
...
--force    # re-apply all patches, even if already applied
```

See [patches configuration](/docs/configuration#patches) for how to configure patches.
