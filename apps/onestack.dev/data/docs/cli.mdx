...
Manually apply package patches configured via the `deps` option in your Vite config. Patches are automatically applied during `one dev` and `one build`, but you can run this command to apply them manually.
...
See [deps configuration](/docs/configuration#deps) for how to configure patches.
