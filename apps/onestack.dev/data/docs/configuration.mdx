...
This makes React Native support easier as we can specify configuration for each environment separately. We define `ios` and `android` environments, respectively. For web, we use the default `client` and `ssr` environments, for client-side and server-side, respectively.

What this means as a user is that you likely want to lean on using `environments` to configure as much as you can in your `vite.config.ts`. It may also mean that some plugins can cause issues by configuring things across all four environments when you really only want it for one. In those cases you'll want to likely wrap the plugin and check for `this.environment` inside functions like `resolveId`, `load`, and `transform`. We're working on a helper to make this easier.
