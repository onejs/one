...
### optimization.warmRoutes

Routes to fetch after the dev server starts to trigger Vite's dependency discovery upfront. Discovered deps are cached in `.vite` and pre-included on next startup, eliminating "optimized dependencies changed. reloading" cascades.

On first run, One fetches each route after the server starts, triggering Vite to discover and optimize all dependencies. These deps are saved to a cache file inside `node_modules/.vite` (which Vite automatically clears when your lockfile or config changes). On subsequent startups, the cached deps are injected into `optimizeDeps.include` so everything is pre-optimized before you open the browser.
...
Add the routes you visit most frequently during development. The cache accumulates — if warming discovers new deps on any run, they're added to the cache for next time.
