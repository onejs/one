...
One is a single Vite plugin exported from `one/vite`, almost all configuration is done through the plugin in your `vite.config.ts`, which takes a single object with entirely optional properties:
    ...
    one({
      ...
      */
      ...
      server: {
        ...
        // defaults to node, vercel and cloudflare are in alpha
        platform: 'node' | 'vercel' | 'cloudflare'
      },
