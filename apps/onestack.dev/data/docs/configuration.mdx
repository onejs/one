...
Turn on [vite-plugin-barrel](https://github.com/JiangWeixian/vite-plugin-barrel). Optimizes barrel export files to speed up your build. You must list the packages that have barrel exports. Especially useful for icon packs.
...
export default {
  plugins: [
    one({
      optimization: {
        barrel: ['@tamagui/lucide-icons', 'my-icon-package'],
      },
    }),
  ],
}
