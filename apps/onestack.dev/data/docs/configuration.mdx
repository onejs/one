  ...
  plugins: [
    one({
      ...
      router: {
        ...
        /**
         * Configure deep linking. Custom schemes get expanded to URL prefixes
         * that are stripped before route matching, and merge with the schemes
         * registered in your native app manifest.
         *
         * The plugin inlines this object into the generated entry as JSON,
         * so only serializable fields are accepted here. For programmatic
         * configuration (including `filter`), call `createApp({ linking })`
         * from a custom entry — see the deep linking guide.
         */
        linking: {
          // one or more custom app schemes — also register these in your
          // native app manifest (app.json `scheme`)
          scheme: 'myapp',
          // fully qualified URL prefixes to strip before route matching
          // (universal links, host-bearing custom scheme URLs)
      ...
      // apply patches to node_modules - the React Native and general node module
      // package ecosystem doesn't always play nice with universal apps
      patches: {
        '@sentry/react-native': {
          version: '~15.2.0',
          '**/*.js': ['jsx'],
...
Per-environment module aliases that work during both Vite transforms and rolldown dep pre-bundling. Unlike Vite's `resolve.alias`, these are applied via `resolveId` hooks so they're respected by the optimizer.

Specify aliases per environment. More specific environments (`client`, `ssr`, `ios`, `android`) override the general ones (`web`, `native`).
...
export default {
  plugins: [
    one({
      alias: {
        // applies to client + ssr environments
        web: {
          'react-native-web': '@tamagui/react-native-web-lite',
        },

        // applies to ios + android environments
        native: {
          'some-package': 'some-package-native',
        },

        // overrides web for ssr only
        ssr: {
          'some-package': 'some-package-ssr',
        },
      },
    }),
  ],
...
Replacement values can be bare specifiers — One resolves them internally.
...
Pass `false` to completely disable native (iOS/Android) support. When disabled, One will not register the `ios`/`android` Vite environments, skip the Metro / React Native dev server plugins, and skip the rolldown native bundle pipeline — useful when you want to use One purely as a web framework.
...
- `'vite'` (alpha): Uses Rolldown for native bundling with fast builds, HMR via React Refresh, and treeshaking in production. Requires Hermes V1.
...
When using Metro mode, you can also configure `bundlerOptions.startup: 'lazy'` to defer Metro startup until a native client connects, which speeds up dev server startup when you don't always need native builds.
...
Note that `native.bundler` only affects dev. `one build --platform ios` and `--platform android` always produce the native bundle through Rolldown.

### native.bundlerOptions

Options for whichever bundler `native.bundler` selects. With `'vite'`:
...
`plugins` are Rolldown plugins run on the native bundle in both dev and build. The native bundler is its own Rolldown instance rather than a Vite environment, so plugins in your top-level `plugins` array never reach it — this is how you add one.

`define` follows Vite's rules: a string value is inserted as a raw expression, anything else is JSON stringified. It's deliberately separate from your Vite `define` rather than inherited from it, because plugins add web-only defines there. `@tamagui/vite-plugin` sets `_WORKLET: false` for Reanimated on web, and inheriting that on native would statically kill every worklet.

With `'metro'`, see the [Metro Mode documentation](/docs/metro-mode) for `nativeTransformModules`, `nativeTransforms`, `startup`, and the Metro config passthroughs.

#### Imported functions in worklets

For Rolldown, `workletImportsPlugin` from `vxrn` makes selected exports from a pure JavaScript package callable inside ordinary worklets:

```tsx fileName=vite.config.ts
import { one } from 'one/vite'
import { workletImportsPlugin } from 'vxrn'
  ...
  plugins: [
    one({
      native: {
        bundler: 'vite',
        bundlerOptions: {
          plugins: [workletImportsPlugin({ remend: ['default'] })],
        },
      },
    }),
  ],
...
Keep your usual `import remend from 'remend'` in application code. The plugin bundles the selected exports and their dependencies into worklets, so they can run on a separate worklet runtime. This is an explicit opt-in that adds bundled code and serialized worklet code for each configured entry.

Select synchronous functions that work without a receiver or native runtime dependencies. The replacement module exposes only the selected exports. Function identity, `name`, `length`, attached properties, and constructor behavior are not preserved. Each entry owns its dependency graph, so module state is not shared with other configured entries or ordinary imports through another path.

Initialization happens lazily once per runtime and live module version. Editing a dependency rebuilds the worklet module during development; retained worklets keep their original version. Restart the dev server after changing the plugin configuration or package export mappings.

Node built-ins, React Native runtime packages, dynamic imports, and unsupported free runtime globals are rejected. The plugin cannot prove that a function is pure: the selected functions and their dependencies must not rely on host objects, runtime-specific properties on `globalThis`, or asynchronous module initialization.
...
This inlines ALL CSS. For per-file control, use the [`.inline.css` extension](/docs/guides-css#inline-css-inlinecss) instead, which only inlines the files you mark.

**Default:** `false`

### Code Splitting

One uses Rolldown for production builds and configures sensible code splitting defaults out of the box. By default, chunks smaller than 42KB are merged together to reduce the number of HTTP requests on page load.

You can customize the code splitting behavior through `build.rolldownOptions.output.codeSplitting`:
...
export default {
  ...
  build: {
    rolldownOptions: {
      output: {
        codeSplitting: {
          // minimum chunk size in bytes before merging (default: 42000)
          minSize: 42_000,

          // maximum chunk size - splits chunks larger than this
          maxSize: 500_000,

          // group specific modules into named chunks
          groups: [
            {
              name: 'vendor',
              test: /node_modules/,
              priority: 1,
              minSize: 50_000,
              // only create chunk if shared by 2+ entry points
              minShareCount: 2,
            },
          ],
        },
...
**Why this matters:** Without chunk merging, a large app can produce hundreds of tiny JS files (many under 1KB). Even with HTTP/2 multiplexing, this many requests saturates browser connections and increases total transfer overhead. The 42KB default strikes a balance between granular caching and fewer round-trips.

**SPA page optimization:** For SPA pages, One skips emitting `<link rel="modulepreload">` hints in the HTML since the JS module graph handles its own lazy loading and route preloading happens on hover intent. This avoids saturating the connection with hundreds of preload hints that provide no benefit for client-rendered pages.
  ...
  plugins: [
    one({
      react: {
        ...
        // or
        compiler: /\/app\//, // enable for files matching regex
        // or
        compiler: (id, env) => {
          // env is 'ios' | 'android' | 'client' | 'ssr'
...
- **`true`**: Enable for all platforms (excluding node_modules, +api files)
- **`'web'`**: Enable only for web (client and SSR)
- **`'native'`**: Enable only for native (iOS and Android)
- **`RegExp`**: Enable for files matching the pattern (full control, no default filtering)
- **`(id, env) => boolean`**: Custom function with file path and environment (`'ios' | 'android' | 'client' | 'ssr'`)
- **`{ web, native }`**: Configure each platform separately with any of the above options

When using `RegExp` or a function, you have full control over which files are compiled - the default exclusions (node_modules, +api files) are not applied. When using `true` (or `true` within the object form), the default filters apply.
...
A standalone Vite plugin that passively tracks dependencies Vite discovers during normal development. On your next dev server startup, those deps are pre-included in `optimizeDeps.include`, eliminating "optimized dependencies changed. reloading" cascades.

Import it from `one/vite` and add it as a top-level plugin:

```tsx fileName=vite.config.ts
import { autoWarmPlugin, one } from 'one/vite'
...
During the first 5 minutes of each dev session, the plugin polls Vite's in-memory optimizer metadata and caches any newly discovered deps to `node_modules/.vite/one-warm-deps.json`. This file clears automatically when Vite clears its cache (lockfile or config changes).

To persist the cache permanently (surviving `node_modules` reinstalls), pass a file path:
...
The `patches` option lets you coerce your node_modules to be compatible with Vite.
...
Scan client bundles for leaked secrets (API keys, tokens, passwords) after building. This catches accidental exposure of server-side secrets in client-facing JavaScript before they reach production.
...
export default {
  plugins: [
    one({
      build: {
        // Log warnings when secrets are found (default)
        securityScan: 'warn',

        // Fail the build if secrets are found (recommended for production)
        securityScan: 'error',
        ...
        securityScan: {
          level: 'error',
          safePatterns: [
            // exact strings that are known safe
            '09259e3b-7be8-46f6-9801-106bf1866e1c',
            // regex patterns for domain-specific matches
            /my-app-test-token-[a-z]+/,
          ],
        },
      },
    }),
...
- **`'error'`**: Fail the build if secrets are detected — **recommended for production CI/CD**
- **`false`**: Disable scanning entirely
- **`{ level, safePatterns }`**: Object form with `level` (`'warn'` or `'error'`) and `safePatterns` (array of strings or RegExp to ignore)

The scanner checks for common secret patterns including Anthropic, OpenAI, Stripe, GitHub, and AWS keys, as well as generic secret/password/token assignments. Known safe patterns (WebRTC constants, CSS tokens) are automatically excluded. Use `safePatterns` to add your own.
...
Configure environment guard behavior for `server-only`, `client-only`, `native-only`, and `web-only` imports.

One enforces these guard imports to prevent modules from being used in the wrong build environment. However, some libraries (like `floating-ui`) import `client-only` even though the imported code works fine on the server when you're only importing utilities.
...
export default {
  plugins: [
    one({
      ...
      // Or disable all environment guards entirely
      environmentGuards: {
        disabled: true,
      },
    }),
...
- **`disabled`**: When `true`, disables all environment guard checks. All guard imports become no-ops regardless of environment.
- **`disableGuards`**: An array of specific guard types to disable. Options are `'server-only'`, `'client-only'`, `'native-only'`, and `'web-only'`.

**Default:** All guards are enabled.
