    ...
    one({
      ...
      */
      ...
      native: {
        ...
        // Configure bundler-specific options
        bundlerOptions: {
          // Metro-specific configuration when bundler is 'metro'
        }
      },
...
You can also enable Metro mode via environment variable:

```bash
ONE_METRO_MODE=1 npm run dev
```

See the [Metro Mode documentation](/docs/metro-mode) for detailed information.

### native.bundlerOptions

Configure bundler-specific options. When using Metro mode, you can pass Metro configuration options here. Note that options merging is not fully implemented yet.
