...
One uses up to eight workers by default. Set `build.concurrency` when the build shares a machine with other CPU-heavy work:
...
`ONE_BUILD_CONCURRENCY` overrides the config value for one-off builds:

```bash
ONE_BUILD_CONCURRENCY=2 npx one build
```

You can also disable worker threads entirely through config or an environment variable:
