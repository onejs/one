...
`useStack` is a state reader. To navigate, use [`useRouter`](/docs/hooks-useRouter) / `router` and [`Link`](/docs/components-Link) as you would anywhere else in the app - every entry carries an `href`.
...
Screens with a `presentation` option (`sheet`, `modal`, `formSheet`, ...) still show up in `screens` alongside regular pushed screens - `useStack` doesn't separate them out. If your custom layout needs to treat them differently, check `focused.options.presentation`. Most apps handle presentation screens with [Presentations](/docs/components-Presentations) instead, which is scoped specifically to them.
