...
Second, One ensures that all environment variables loaded are only available to the server-side functionality, unless they are prefixed with `VITE_` (Vite convention), `EXPO_PUBLIC_` (Expo convention), or `ONE_PUBLIC_`.
...
`ONE_PUBLIC_` is One's default naming convention, while `EXPO_PUBLIC_` remains fully supported. A `ONE_PUBLIC_` variable is also exposed under the matching `EXPO_PUBLIC_` name when that Expo variable is absent. One does not create `ONE_PUBLIC_` variables from Expo variables, and an explicitly defined Expo variable keeps its own value.
...
- `ONE_PORT` sets the port for both `one dev` and `one serve`. This is equivalent to passing `--port` on the command line, but works across both commands and can be set in your `.env` file or deployment config. If `--port` is also passed, the CLI flag takes precedence.
- `ONE_FORCE_PORT` overrides the port for both `one dev` and `one serve`, taking precedence over `--port` and `ONE_PORT`. Useful for CI or deployment environments where the port must be fixed regardless of other configuration.
...
2. `--port` CLI flag
3. `ONE_PORT`
4. Built-in defaults (8081 for dev, 3000 for serve)
...
- `VITE_NATIVE` is `"1"` for native platforms (iOS and Android), or `""` for web (client and SSR). Uses truthy/falsy strings to avoid polluting `process.env` types. Useful for tree-shaking native-only code.
- `ONE_PLATFORM` is `"web"`, `"ios"`, or `"android"`. Expo packages also receive `EXPO_OS` as `"ios"` or `"android"` in native builds; One does not define it on web.
...
This gives you autocomplete and type checking for `import.meta.env.VITE_ENVIRONMENT`, `process.env.VITE_NATIVE`, and all other One environment variables.
