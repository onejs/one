...
`ONE_PUBLIC_` is One's default naming convention, while `EXPO_PUBLIC_` remains fully supported. When only one form is defined, One also exposes the matching form with the same suffix and value. If both forms are explicitly defined, their values remain independent.
...
- `ONE_PLATFORM` and its Expo-compatible alias `EXPO_OS` are set to `"web"` for web builds (both client and SSR), or `"ios"` / `"android"` for native platforms.
