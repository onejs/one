...
Layouts can also specify render modes using the same suffix pattern:

- `_layout+ssg.tsx` - Layout is pre-rendered at build time (static)
- `_layout+ssr.tsx` - Layout is rendered on the server per-request
- `_layout+spa.tsx` - Layout is client-rendered only (default behavior)

**Why this matters for SPA pages:**

When an SPA page has a parent layout with `+ssg` or `+ssr` render mode, the layout shell is rendered on the server while the page content remains client-rendered. This gives users immediate visual content (navigation, sidebar, head tags, CSS) without waiting for JavaScript to download and execute.

```
app/
├── _layout+ssg.tsx      # Root layout rendered at build time
├── dashboard+spa.tsx    # SPA page - but layout shell is pre-rendered!
└── settings+spa.tsx     # Same - instant layout, client page content
```

This is the recommended approach for SPA-heavy apps that still want fast initial paint. The layout's HTML is included in the initial response, and page content hydrates after JavaScript loads.

### Folder-Level Render Modes

You can also apply render modes to entire folders, and all routes within that folder will inherit the render mode. This is useful for organizing routes by their rendering strategy:
...
1. **File suffix** (highest priority) - `page+spa.tsx`
2. **Parent folder suffix** - `dashboard+ssr/page.tsx`
3. **Vite configuration** (lowest priority) - `defaultRenderMode: 'ssg'`

**Nested folders:**
Inner folder suffixes override outer folder suffixes:
...
**Important notes:**
- Folder suffixes are stripped from URLs: `dashboard+ssr/analytics` becomes `/dashboard/analytics`
- File suffixes always take precedence over folder suffixes
- Layouts do not inherit folder render modes (they remain as layouts)

**API routes benefit too:**
...
Its downsides include a slower initial load time, and worse SEO. You can mitigate this by using a `_layout+ssg.tsx` or `_layout+ssr.tsx` root layout, which renders your layout shell on the server while keeping page content client-rendered. See [Layout Render Modes](#layout-render-modes) above.
...
API routes use Web standard [Request](https://developer.mozilla.org/en-US/docs/Web/API/Request) and [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) objects.

See [API Routes](/docs/routing-api-routes) for full documentation.
