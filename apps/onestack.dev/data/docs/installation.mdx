...
The generated Metro project uses `@react-native/babel-preset` and `one/babel-preset`.
Those Babel settings belong to the Metro path and are not part of One's default bundler.
