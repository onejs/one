...
One's default native path uses its Vite and Rolldown pipeline and does not require Expo or
a Babel config.
...
Metro remains available for packages that require it. Set `bundler: 'metro'` in the native
configuration and generate its explicit config:

```bash
bunx one metro-eject
```

The generated `babel.config.cjs` delegates to the preset (`module.exports = require('one/babel-preset')`),
which builds on React Native's preset. Those Babel settings belong to the Metro path and are not
part of One's default bundler.
...
- **Production compatibility**: A familiar path for packages designed around Metro

Use Metro when a dependency requires it. See the [Metro Mode documentation](/docs/metro-mode) for more details.
