...
The native app manifest registers URLs with iOS and Android. `router.linking` tells One how to strip those URLs before matching routes, so configure the scheme in both places.
...
The scheme in `native.app` lands in `Info.plist` after running `one prebuild`. To verify:
...
`one prebuild` registers the `native.app` scheme in `AndroidManifest.xml`. Add universal-link hosts to the generated native project, then test with:
