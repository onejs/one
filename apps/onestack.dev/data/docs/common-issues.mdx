...
If you do find the individual module that is causing an issue, you can also configure it quickly using the `patches` option in the [One Vite plugin configuration](/docs/configuration#patches).

### Uncaught SyntaxError: The requested module ... does not provide an export named 'default'

In general you want to add any dependency like this to `optimizeDeps.exclude` in your Vite config.

### `require` is not defined

You can try using one of the commonjs plugins, either `vite-require` or `vite-plugin-commonjs`.
...
For more details, see the [`patches` section in Configuration](/docs/configuration#patches).
...
If your React Native app is erroring on startup, you need to make sure your One Vite plugin is configured to
