...
The mechanical rule: **a navigator child that is not config (`Screen`, `Protected`, `Stack.Header`) replaces the headless default on web, and is ignored on native.** The same layout file works on both platforms. Native keeps rendering its usual chrome from the `Screen` config, while web renders `WebTabBar` instead. If a custom layout ever needs a different native version too, split it into a `.web.tsx` file the same way you would any other platform-specific component.
...
Native is unaffected by any of this. `<Stack>`, `<Tabs>`, and `<Drawer>` still render through React Navigation with real headers, a real bottom tab bar, and a real drawer. Nothing about your `Screen` config, `screenOptions`, or `Stack.Header` composition changes.
