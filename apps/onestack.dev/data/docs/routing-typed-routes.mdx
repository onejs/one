...
One also provides TypeScript types for [preset environment variables](/docs/environment#preset-environment-variables). Add this to any `.d.ts` file in your project:

```ts
/// <reference types="one/env" />
```

This gives you typed access to `import.meta.env.VITE_PLATFORM`, `process.env.VITE_ENVIRONMENT`, and other One environment variables.
