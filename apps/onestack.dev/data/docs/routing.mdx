...
You can choose a rendering strategy on a per-page basis using a filename suffix. For more on these modes, [see the next documentation page, Render Modes](/docs/render-modes).
...
Layouts also support render mode suffixes (`_layout+ssg.tsx`, `_layout+ssr.tsx`). This is especially useful for SPA-heavy apps - when a parent layout has SSG or SSR mode, the layout shell renders on the server while page content remains client-rendered. See [Layout Render Modes](/docs/routing-layouts#layout-render-modes) for details.
