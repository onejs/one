...
function OrderStatus({ progress }: { progress: string }) {
  return (
...
`WidgetUI` supports `Text`, `VStack`, `HStack`, and `Spacer`, with `color`,
`backgroundColor`, `fontSize`, `fontWeight`, `padding`, `borderRadius`, and
`spacing` styles. Colors use six-digit hex values. JSX layouts can contain pure
