...
the artwork, then generates density-specific Android assets inside a 288 dp square.
...
For JSX layouts, install `@use-voltra/ios-client` and add `jsx` to the same
`ios.widgets` object. One's prebuild includes Voltra's SwiftUI renderer in its
existing extension target. React renders the JSX layout to a serialized payload
in the app; the extension reads and displays that payload without the app runtime.

```ts
jsx: {
  id: 'order_status',
  displayName: 'Order status',
  description: 'An order widget authored in JSX.',
},
```

```tsx
import { Voltra, updateWidget, startLiveActivity } from '@use-voltra/ios-client'

const view = <Voltra.VStack><Voltra.Text>Order 2 of 3</Voltra.Text></Voltra.VStack>
await updateWidget('order_status', { systemSmall: view, systemMedium: view })
await startLiveActivity({ lockScreen: view })
```

The built-in SwiftUI widget uses the fixed `title`, `value`, and `subtitle` contract.
The optional Voltra widget uses its own JSX payload and widget id. Android widgets
require a separate Glance target, storage contract, and lifecycle; this setting
