...
If you're using [Expo Prebuild](https://docs.expo.dev/workflow/prebuild), you should also add `"one/expo-plugin"` in your app config (app.json/app.config.js/app.config.ts):

```json
{
  "expo": {
    "plugins": ["one/expo-plugin"]
  }
}
```
...
<Notice>
  This isn't necessary if you're using Expo Prebuild, as `one/expo-plugin` already takes care of this.
</Notice>
...
1. Make sure there's a `react-native.config.cjs` in your project which contains something like this:
    ...
    ```
2. Make sure you have `one/expo-plugin` included in your `app.json` (or .js, etc.):
