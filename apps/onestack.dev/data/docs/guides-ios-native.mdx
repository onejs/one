...
**Note**: One builds and runs the native app directly: `one prebuild` generates
the Xcode project from your `native.app` manifest, and `one run:ios` launches
it on a simulator. While the dev server runs, press <kbd>o</kbd> <kbd>i</kbd>
in its terminal to open the app in the iOS simulator.
...
One reads this manifest directly. It generates the iOS and Android app icons and
launch screens from the shared sources, and it does not require an Expo app config
or config plugin. The icon must be a square image at least 1024 pixels wide. Splash
