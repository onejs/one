...
On client-side navigation, layout loaders do not re-run - their data is cached from the initial page load. Only the destination page's loader runs. This is similar to TanStack Router's default behavior and is intentional for performance, as layout data (navigation items, site config) rarely changes.
