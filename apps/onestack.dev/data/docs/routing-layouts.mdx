...
<RouteTree
  routes={[
    {
      name: '_layout.tsx',
      description: 'Wraps all files in this directory and below',
      highlight: true,
    },
    { name: 'index.tsx', description: 'Matches "/"' },
    {
      ...
      children: [
        {
          name: '_layout.tsx',
          description: 'A custom sub-layout for all /blog pages',
          highlight: true,
        },
        { name: 'index.tsx', description: 'Matches "/blog"' },
        {
          name: '[slug].tsx',
          description: 'Matches a single sub-path of "/blog", like "/blog/hello"',
        },
      ],
    },
  ]}
...
On web, Stack, Tabs, and Drawer render headless by default: just the matched route, no header, no tab bar, no sidebar. See the [headless web navigators guide](/docs/guides-headless-web) for how to build your own web chrome.
...
See [useMatches](/docs/hooks-useMatches) for more details.
...
If you need to access the current route params in a layout, you can use [the useActiveParams hook](/docs/hooks-useActiveParams).
...
```tsx fileName=app/_layout+ssg.tsx
import { Slot } from 'one'
