...
export const POST = async (req: Request): Promise<Response> => {
  try {
    if (!req.headers.get("Content-Type")?.includes("multipart/form-data")) {
      return new Response(JSON.stringify({ status: "error", message: "Invalid content type" }), {
        status: 400,
    ...
    return new Response(JSON.stringify({ status: "error", message: signupResult.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
