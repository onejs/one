...
CONTEXT
`one` at /Users/n8/.worktrees/one-native/packages/one contains generated SwiftUI bindings for React Native Fabric components (OneNativeMenu, OneNativeTabs, OneNativePicker, OneNativeToggle, OneNativeSheet, etc.) and exports a machine-readable description of every component at packages/one/schema.json (components with props/events/slots, payload shapes, enums, and a controlledProtocol block).
...
5. /Users/n8/.worktrees/one-native/packages/one/schema.json (read it fully; it is ~12KB).
...
F. Whether `one` is referenced anywhere else in ~/soot besides that one seam file.
