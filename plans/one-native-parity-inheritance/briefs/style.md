Build the styling surface for packages/one. This is the largest gap in the package: there is
...
Prove the gap for yourself first, it takes a minute: grep the catalogs in packages/one/codegen
...
behind the SwiftUI content, not the content itself. See packages/one/codegen/emitControls.ts
...
payload applied once by a generated modifier:
  packages/one/ios/OneNativeAccessibility.swift   a struct plus `extension View { func oneNativeAccessibility(_:) }`
