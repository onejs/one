...
lives in `packages/one-native`; `tests/native-features/app/one-native.tsx` is the
native integration fixture. The broader generated SwiftUI engine below remains a
proposal. This package does not yet replace the navigation, toolbar, zoom, and
split-view integrations in `@vxrn/native`.

**Implemented foundation**

- `Swift.Tabs` and keyed `Swift.Tab` pages use an owned Fabric component and a
  SwiftUI `TabView`, with no navigator or bottom-tabs package dependency.
- A `UIViewRepresentable` slot hosts each RN page. SwiftUI allocates the bounded
  page rectangle and updates a custom Fabric state; Yoga sizes the descendants.
  Only changed geometry is published. This is a bounded page contract, not yet
  general content-sized SwiftUI/RN layout negotiation.
- Native selection is optimistic and numbered. The React adapter acknowledges
  each callback with its current controlled selection, including rejection. Older
...
asserted mounted app/page content before interaction and checked:

- React selection updates the visible native page; native taps update React.
- Rejecting a native tap restores the controlled page and its accessibility.
- Counters and text input survive switching and keyed tab reorder; both pages
  still render after reorder.
- Native nested menu actions reach React, disabled actions are disabled, hidden
...
rectangle changes. Tab metadata rebuilds only after changed metadata or child
mounts. Menu flattening and native grouping are linear in the item count; immutable
item arrays let unchanged renders reuse their payload. No performance benchmark
or payload-size claim is made yet.
