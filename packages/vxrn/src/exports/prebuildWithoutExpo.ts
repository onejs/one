...
function patchIosAppDelegateUpdatesBundleURL(rendered: string): string {
  // the c entry point arrives through the app's bridging header: app
  // targets cannot import One, whose c++ umbrella does not rebuild under
  // their flags.
  const bundleAnchor = `#else
    Bundle.main.url(forResource: "main", withExtension: "jsbundle")
#endif`
  if (!rendered.includes(bundleAnchor)) {
    throw new Error(
      '[vxrn] prebuild template AppDelegate.swift changed shape: cannot point the release bundleURL at One.Updates'
    )
  }
  return rendered.replace(
    bundleAnchor,
...
const UPDATES_BRIDGING_HEADER_NAME = 'OneUpdates-Bridging-Header.h'

function renderUpdatesBridgingHeader(): string {
  return `// generated by one prebuild: exposes the updates launcher entry
// point to AppDelegate without importing the One module.
#import "OneUpdatesLauncherBridge.h"
`
}

function generateUpdatesBridgingHeader(dest: string, app: NativeAppManifest): void {
  if (app.updates === undefined) return
  FSExtra.writeFileSync(
    path.join(dest, app.name, UPDATES_BRIDGING_HEADER_NAME),
    renderUpdatesBridgingHeader()
  )
}

function patchIosPbxprojUpdatesBridgingHeader(project: string, appName: string): string {
  ...
  const setting = `SWIFT_OBJC_BRIDGING_HEADER = "${appName}/${UPDATES_BRIDGING_HEADER_NAME}";`
  const occurrences = project.split(anchor).length - 1
  if (occurrences !== 2) {
    throw new Error(
      '[vxrn] prebuild template project.pbxproj changed shape: cannot point the app target at the One.Updates bridging header'
    )
  }
  return project
    .split(anchor)
...
${schemes.map((scheme) => `            <data android:scheme="${scheme}" />`).join('\n')}
    ...
    if (platform === 'ios' && relativePath.endsWith('/AppDelegate.swift')) {
      ...
      if (app.updates !== undefined) {
        rendered = patchIosAppDelegateUpdatesBundleURL(rendered)
      }
    }
    ...
    if (platform === 'ios' && relativePath.endsWith('.xcodeproj/project.pbxproj')) {
      ...
      if (app.updates !== undefined) {
        rendered = patchIosPbxprojUpdatesBridgingHeader(rendered, appName)
      }
      if (app.notifications?.push === true && !app.ios?.widgets) {
...
export const generateForPlatform = async (
  ...
  if (platform === 'ios') generateUpdatesBridgingHeader(dest, app)
