...
export const create = async (options: StartOptions) => {
	...
	const virtualModules = {
		"react-native": {
			...
			contents: await readFile(
				require.resolve("@vxrn/react-native-prebuilt"),
				"utf-8",
			),
		},
		react: {
	...
	console.log(
		Object.fromEntries(
			Object.entries(virtualModules).map(([k, v]) => [k, v.alias]),
		),
	);
	...
	return {
		...
		android: {
			...
			adaptiveIcon: {
				...
				backgroundColor: "#FFFFFF",
				foregroundImageUrl:
					"http://127.0.0.1:8081/assets/./assets/adaptive-icon.png",
			},
		},
