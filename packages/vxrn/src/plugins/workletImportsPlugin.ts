...
export function workletImportsPlugin(imports: Record<string, readonly string[]>): Plugin {
  ...
  let nativeOptions: InputOptions
  ...
  return {
    ...
    async resolveId(source, importer, options) {
      ...
      if (
        ...
        throw new Error(`[worklet imports] ${source} must resolve to a JavaScript file`)
      }
        ...
        plugins: [
          {
            ...
            moduleParsed(info) {
              ...
              if (info.packageJsonPath) outerContext.addWatchFile(info.packageJsonPath)
            },
          },
