...
class OneZoomManager {
  static let shared = OneZoomManager()
...
class ZoomTransitionsSourceRepository {
  ...
  func registerSource(identifier: String, source: SourceInfo) {
    ...
    if sources[identifier] != nil {
      NSLog("[one] ZoomSource with identifier %@ is already registered.", identifier)
    }
...
class ZoomTransitionSourceView: RCTView {
  ...
  private var sourceRepository: ZoomTransitionsSourceRepository { OneZoomManager.shared.sourceRepository }
  ...
  public override func insertReactSubview(_ subview: UIView!, at atIndex: Int) {
    guard child == nil else {
      NSLog("[one] ZoomSource can only have a single native child.")
      return
    }
    child = subview
    sourceRepository.registerSource(
...
class ZoomTransitionAlignmentRectDetectorView: RCTView {
  private var child: UIView?
  private var alignmentViewRepository: ZoomTransitionsAlignmentViewRepository { OneZoomManager.shared.alignmentViewRepository }
  ...
  @objc var identifier: NSString = "" {
    didSet {
      ...
      if oldId != newId && !oldId.isEmpty {
        NSLog("[one] AlignmentRectDetector does not support changing identifier.")
        return
      }
      _identifier = newId
      if let child = child {
  ...
  public override func insertReactSubview(_ subview: UIView!, at atIndex: Int) {
    guard child == nil else {
      NSLog("[one] ZoomTarget can only have a single native child.")
      return
    }
...
class ZoomTransitionEnablerView: RCTView {
  ...
  private func setupZoomTransition() {
    if _zoomTransitionSourceIdentifier.isEmpty {
      NSLog("[one] No zoomTransitionSourceIdentifier passed to ZoomTransitionEnabler.")
      return
    }
    if let controller = self.findViewController() {
      NSLog("[one] Found VC: %@ (class: %@) for zoom id: %@", controller.description, String(describing: type(of: controller)), _zoomTransitionSourceIdentifier)
      if #available(iOS 18.0, *) {
        ...
        controller.preferredTransition = .zoom(options: options) { _ in
          ...
          guard let view = sourceInfo?.view else {
            NSLog("[one] No source view found for identifier %@.", self._zoomTransitionSourceIdentifier)
            return nil
          }
          NSLog("[one] Zoom source view found: %@ for id: %@", view.description, self._zoomTransitionSourceIdentifier)
          return view
        }
        NSLog("[one] preferredTransition set to .zoom for VC: %@", controller.description)
        return
      }
    } else {
      NSLog("[one] No navigation controller found to enable zoom transition.")
    }
  }
