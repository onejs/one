...
import { flattenMenuItems } from '../src/platform/menuItems'
...
describe('native menu payload', () => {
  ...
  it('gates ButtonRole.confirm and TabRole.prominent by OS via assertSwiftUIValue', () => {
    ...
    expect(
      flattenMenuItems([{ type: 'action', id: 'del', title: 'Delete', role: 'destructive' }])[0]
    ).toMatchObject({ role: 'destructive' })
    expect(() =>
      flattenMenuItems([{ type: 'action', id: 'ok', title: 'OK', role: 'confirm' }])
    ).toThrow('ButtonRole.confirm requires iOS 26')
    expect(
      flattenMenuItems([{ type: 'action', id: 'ok', title: 'OK', role: 'confirm' }], 26)[0]
