...
async function loadNative() {
  vi.resetModules()
  return import('../src/platform/app-info/index.native')
}

async function loadWeb() {
  vi.resetModules()
  return import('../src/platform/app-info/index')
}
...
describe('app-info native snapshot', () => {
  it('reads the hybrid object properties', async () => {
    hybridMock.mockReturnValue({
      version: '9.9.9',
      build: '4242',
      applicationId: 'dev.onejs.one.tests',
    })
  ...
  it('nulls fields the platform leaves undefined', async () => {
    hybridMock.mockReturnValue({ version: '9.9.9', applicationId: 'dev.onejs.one.tests' })
    const { AppInfo } = await loadNative()
    expect(AppInfo).toEqual({
      version: '9.9.9',
      build: null,
...
describe('cold dev boot', () => {
  ...
  it('bundles the app-info web entry without native-only imports', async () => {
    const result = await build({
      entryPoints: [join(packageDir, 'src', 'platform', 'app-info', 'index.ts')],
      bundle: true,
      write: false,
      platform: 'browser',
      format: 'esm',
