...
const specsDir = join(packageDir, 'src', 'platform', 'specs')
...
describe('native bundle artifact', () => {
  it('resolves, bundles, and registers a static spec through buildNativeBundle', async () => {
    ...
    for (const relative of [
      'esm/platform/specs/OneNativeMenuNativeComponent.mjs',
      'esm/platform/specs/OneNativeMenuNativeComponent.native.js',
    ]) {
    ...
    await writeFile(
      join(stage, 'entry.js'),
      `import spec from './dist/esm/platform/specs/OneNativeMenuNativeComponent'\nglobalThis.__loadedSpec = spec\n`
    )
  ...
  it('ships both raw specs and static dist for pack consumers', () => {
    ...
    expect(existsSync(join(packageDir, 'src', 'platform', 'specs', 'OneNativeMenuNativeComponent.ts'))).toBe(
      true
    )
  })
})
