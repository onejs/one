...
} from '../src/platform/safe-area/insets'
...
describe('native safe-area module', () => {
  it('registers the provider host under its Fabric name', async () => {
    const { NativeSafeAreaProvider } = await import('../src/platform/safe-area/index.native')
  ...
  it('reads synchronous constants from a TurboModule-shaped module', async () => {
    ...
    const { getInitialWindowMetrics } = await import('../src/platform/safe-area/index.native')
...
describe('cold dev boot', () => {
  ...
  it('bundles the safe-area-context web entry without native-only imports', async () => {
    const result = await bundleWebEntry(
      join(packageDir, 'src', 'safe-area-context', 'index.ts')
    )
    expect(result.outputFiles.length).toBeGreaterThan(0)
  })

  it('bundles the platform safe-area web entry without the Fabric spec', async () => {
    const result = await bundleWebEntry(join(packageDir, 'src', 'platform', 'safe-area', 'index.ts'))
    expect(result.outputFiles.length).toBeGreaterThan(0)
  })
})
