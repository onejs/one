...
__DEV__ &&
  (function () {
    ...
    function accumulateDirectionalDispatches$1(inst, phase, event) {
      ...
      if (
        (phase = getListener$1(inst, event.dispatchConfig.phasedRegistrationNames[phase]))
      )
    ...
    function accumulateDirectionalDispatches(inst, phase, event) {
      inst || error$jscomp$0('Dispatching inst must not be null')
      if (
        (phase = getListener(inst, event.dispatchConfig.phasedRegistrationNames[phase]))
      )
        ((event._dispatchListeners = accumulateInto(event._dispatchListeners, phase)),
          (event._dispatchInstances = accumulateInto(event._dispatchInstances, inst)))
    }
    ...
    function dispatchEvent(target, topLevelType, nativeEvent) {
      ...
      batchedUpdates$1(function () {
        ...
        if (event) {
          ...
          if (hasError)
            throw ((event = caughtError), (hasError = !1), (caughtError = null), event)
        }
      })
    }
    ...
    function getHighestPriorityLanes(lanes) {
      ...
      switch (lanes & -lanes) {
        ...
        default:
          return (
            error$jscomp$0('Should have found matching lanes. This is a bug in React.'),
            lanes
          )
      }
    }
    ...
    function mountSyncExternalStore(subscribe, getSnapshot) {
      ...
      mountEffect(subscribeToStore.bind(null, fiber, cachedSnapshot, subscribe), [
        subscribe,
      ])
      fiber.flags |= 2048
      pushEffect(
    ...
    function updateSuspenseComponent(current, workInProgress, renderLanes) {
      ...
      if (null !== nextPrimaryChildren && null !== nextPrimaryChildren.dehydrated) {
        if (didSuspend)
          ...
          (pushPrimaryTreeSuspenseHandler(workInProgress), isSuspenseInstanceFallback())
        )
