...
#import "OneNativeConfirmationDialogComponentView.h"
...
- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    ...
    _nativeView.onChange = ^(BOOL value, NSInteger eventCount, NSInteger revision) {
      ...
      emitter->onNativeConfirmationDialogValueChange({.value = (bool)value, .eventCount = (int)eventCount, .revision = (int)revision});
    };
    _nativeView.onAction = ^(NSString *id, NSString *presenting, NSInteger eventCount) {
      OneNativeConfirmationDialogComponentView *strongSelf = weakSelf;
      if (!strongSelf || !strongSelf->_eventEmitter) return;
      auto emitter = std::static_pointer_cast<const OneNativeConfirmationDialogEventEmitter>(strongSelf->_eventEmitter);
      emitter->onNativeConfirmationDialogAction({.id = std::string(id.UTF8String), .presenting = std::string(presenting.UTF8String), .eventCount = (int)eventCount});
    };
  }
  return self;
}
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps {
  const auto &next = *std::static_pointer_cast<const OneNativeConfirmationDialogProps>(props);
  const auto &previous = *std::static_pointer_cast<const OneNativeConfirmationDialogProps>(_props);
  ...
  [_nativeView configure:next.value
    acknowledgedEvent:next.acknowledgedEvent revision:next.revision title:RCTNSStringFromString(next.title) message:RCTNSStringFromString(next.message) presenting:RCTNSStringFromString(next.presenting) hasPresenting:next.hasPresenting titleVisibility:RCTNSStringFromString(next.titleVisibility)];
