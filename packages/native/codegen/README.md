...
Public zero-argument `View` methods without generic requirements are now derived as boolean enable props. This adds 17 names, including `hidden`, `labelsHidden`, and `compositingGroup`, with availability checks and framework imports for overlay methods. The next class is bridgeable closure and binding signatures; a shared event and binding transport must carry their values and callbacks without a per-method adapter.
...
`bun run coverage` reports 169 bound/generated modifier names out of 522 public `View` modifier names in the iOS 27 SDK (32%). It counts module/name pairs rather than overloads, across SwiftUI, SwiftUICore, and every installed SwiftUI overlay. Of the 169 bound names, 129 are selected from SDK signatures without a per-name catalog entry; the rest have semantic mappings. Coverage was 51 bound names before this work, then 149 with the iOS 26 ceiling, 152 at the iOS 27 ceiling, and 169 after parameterless methods. The three newly included iOS 27 modifiers are `defaultTabBarPlacement`, `presentationPlacement`, and `textInputBorderShape`. View coverage rose from 32 to 34 with `EditButton` and `EmptyView`.
