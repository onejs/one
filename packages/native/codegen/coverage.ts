...
import { available, readInventory, type Declaration } from './inventory'
...
const isView = (declaration: Declaration) =>
  declaration.kind === 'struct' &&
  declaration.owner === '' &&
  !declaration.name.startsWith('_') &&
  available(declaration)

const isViewModifier = (declaration: Declaration) =>
  declaration.kind === 'func' &&
  shortOwner(declaration) === 'View' &&
  !declaration.name.startsWith('_') &&
  available(declaration)

const emptySet = (): SetCoverage => ({ mapped: 0, total: 0, mappedNames: [], unmappedNames: [] })

export function computeCoverage(
  inventory: readonly Declaration[],
  coverage: ManifestCoverage,
  modules: readonly string[]
): Omit<CoverageReport, 'sdk' | 'declarations'> {
  const universe = new Map<string, { views: Set<string>; modifiers: Set<string> }>()
  for (const module of modules) universe.set(module, { views: new Set(), modifiers: new Set() })
  for (const declaration of inventory) {
    const sets = universe.get(declaration.module)
    if (!sets) continue
    if (isView(declaration)) sets.views.add(declaration.name)
    else if (isViewModifier(declaration)) sets.modifiers.add(declaration.name)
  }
  ...
  for (const [module, sets] of universe) {
    const views = diff(sets.views, coverage.views[module] ?? [])
    const modifiers = diff(sets.modifiers, coverage.modifiers[module] ?? [])
...
function diff(universe: Set<string>, covered: readonly string[]): SetCoverage {
  ...
  return { mapped: mappedNames.length, total, mappedNames, unmappedNames }
}
...
if (manifest.sdk !== sdk) {
  console.error(
    `warning: manifest records SDK ${manifest.sdk} but the toolchain reports ${sdk}; regenerate first`
  )
}
const { modules: byModule, totals } = computeCoverage(inventory, manifest.coverage, modules)
...
if (process.argv.includes('--json')) {
  ...
  console.log(
    `\nSDK ${report.sdk}: ${report.declarations} declarations. views are top-level structs, modifiers are View funcs, both minus SPI; View conformance is not derivable from syntax.`
  )
  }
}
