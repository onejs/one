...
class OneNativeNotificationsModule(reactContext: ReactApplicationContext) :
    ...
    @ReactMethod
    fun getDevicePushToken(promise: Promise) {
        ...
        OneNativePush.getToken(
            ...
            onResult = { token ->
                if (token.isNullOrEmpty()) {
                    promise.reject(E_PUSH_TOKEN, "fetching the push token returned nothing")
                } else {
                    promise.resolve(
                        Arguments.createMap().apply {
                            putString("type", "android")
            ...
            onError = { error ->
                promise.reject(E_PUSH_TOKEN, error.message ?: "fetching the push token failed")
            }
        )
    }
    ...
    companion object {
        ...
        const val EVENT_PUSH_TOKEN = "oneNativeNotificationsPushToken"
        ...
        // rotates. without a live bridge there is nobody to tell; the next
        // getDevicePushToken reads the rotated token instead.
