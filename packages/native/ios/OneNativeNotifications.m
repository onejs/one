...
static NSString *const OneNativeNotificationsPushToken = @"oneNativeNotificationsPushToken";

// forwarded by the prebuild app delegate (which owns the
// UIApplicationDelegate callbacks) when the os answers a remote
// registration. the module observes these; without a register call they
// never fire.
static NSString *const OneNativePushTokenDidRegister = @"OneNativePushTokenDidRegister";
static NSString *const OneNativePushTokenDidFail = @"OneNativePushTokenDidFail";
...
@property (nonatomic, strong) NSMutableArray<NSDictionary *> *pendingPushToken;
@property (atomic, assign) BOOL hasObservers;
+ (instancetype)shared;
+ (BOOL)notificationsEnabled;
+ (NSString *)hexToken:(NSData *)deviceToken;
- (void)presentRequest:(NSString *)requestId
               banner:(BOOL)banner
                 list:(BOOL)list
                sound:(BOOL)sound
                badge:(BOOL)badge;
- (void)pushTokenRegistered:(NSData *)deviceToken;
- (void)pushTokenFailed:(NSString *)message;
@end
...
+ (instancetype)shared
{
  ...
  dispatch_once(&once, ^{
    ...
    shared.pendingPushToken = [NSMutableArray array];
  });
  return shared;
}

// apns device token bytes as lowercase hex, matching expo's token shape.
+ (NSString *)hexToken:(NSData *)deviceToken
{
...
- (void)pushTokenRegistered:(NSData *)deviceToken
{
  self.pushToken = [OneNativeNotificationsDelegate hexToken:deviceToken];
  NSArray *waiting = [self.pendingPushToken copy];
  [self.pendingPushToken removeAllObjects];
  for (NSDictionary *pending in waiting) {
    void (^resolve)(id) = pending[@"resolve"];
    if (resolve) {
      resolve(@{@"type" : @"ios", @"data" : self.pushToken});
    }
  }
}
...
- (void)pushTokenFailed:(NSString *)message
{
  ...
  [self.pendingPushToken removeAllObjects];
  for (NSDictionary *pending in waiting) {
    void (^reject)(NSString *, NSString *, NSError *) = pending[@"reject"];
    if (reject) {
      reject(@"E_NOTIFICATIONS_PUSH_TOKEN",
             message ?: @"registering for remote notifications failed", nil);
    }
  }
}
  ...
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleDidRespond:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handlePushTokenRegistered:)
                                                 name:OneNativePushTokenDidRegister
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handlePushTokenFailed:)
                                                 name:OneNativePushTokenDidFail
                                               object:nil];
  }
  return self;
}
...
- (NSArray<NSString *> *)supportedEvents
{
  return @[
    ...
    OneNativeNotificationsPushToken,
  ];
}
...
// the os answering does.
- (void)handlePushTokenRegistered:(NSNotification *)note
{
  NSData *deviceToken = note.userInfo[@"deviceToken"];
  if (![deviceToken isKindOfClass:[NSData class]]) {
    return;
  }
  [[OneNativeNotificationsDelegate shared] pushTokenRegistered:deviceToken];
  NSString *cached = [OneNativeNotificationsDelegate shared].pushToken;
  if (cached) {
    [self sendEventWithName:OneNativeNotificationsPushToken
                       body:@{@"type" : @"ios", @"data" : cached}];
  }
}

- (void)handlePushTokenFailed:(NSNotification *)note
{
  NSString *message = note.userInfo[@"error"];
  if (![message isKindOfClass:[NSString class]]) {
    message = nil;
  }
  [[OneNativeNotificationsDelegate shared] pushTokenFailed:message];
}
...
// getter waits for the os callback the app delegate forwards. refreshes fan
// out to push token listeners through handlePushTokenRegistered.
RCT_EXPORT_METHOD(getDevicePushToken:(RCTPromiseResolveBlock)resolve
                  reject:(RCTPromiseRejectBlock)reject)
{
  NSString *cached = [OneNativeNotificationsDelegate shared].pushToken;
  if (cached) {
    resolve(@{@"type" : @"ios", @"data" : cached});
    return;
  }
  [[OneNativeNotificationsDelegate shared].pendingPushToken
      addObject:@{@"resolve" : [resolve copy], @"reject" : [reject copy]}];
