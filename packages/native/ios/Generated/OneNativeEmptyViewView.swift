...
private final class EmptyViewModel: ObservableObject {

  @Published var accessibility = OneNativeAccessibility()
  @Published var swiftStyle = OneNativeStyle()
  var active = false
}
@objcMembers public final class OneNativeEmptyViewView: UIView, OneNativeComposable {
  private var model = EmptyViewModel()
  public var onHeight: ((CGFloat) -> Void)?
  private var controller: OneNativeHostingController<OneNativeMeasuredStandalone<EmptyViewContent>>?
  ...
  public func compositionContent() -> AnyView { AnyView(EmptyViewContent(model: model)) }
  ...
  private func updateHost() {
    ...
    if controller == nil {
      bindCallbacks()
      controller = OneNativeHostingController(rootView: OneNativeMeasuredStandalone(content: EmptyViewContent(model: model), onHeight: { [weak self] height in self?.onHeight?(height) }))
    }
  ...
  public func reset() {
    ...
    model.active = false
    controller?.detach(); controller = nil; model = EmptyViewModel()
  }
}
private struct EmptyViewContent: View {
  @ObservedObject var model: EmptyViewModel
  var body: some View {
    EmptyView()
