...
private final class EllipseModel: ObservableObject {
...
@objcMembers public final class OneNativeEllipseView: UIView, OneNativeComposable {
  private var model = EllipseModel()
  public var onHeight: ((CGFloat) -> Void)?
  private var controller: OneNativeHostingController<OneNativeMeasuredStandalone<EllipseContent>>?
  ...
  public func compositionContent() -> AnyView { AnyView(EllipseContent(model: model)) }
  ...
  private func updateHost() {
    ...
    if controller == nil {
      bindCallbacks()
      controller = OneNativeHostingController(rootView: OneNativeMeasuredStandalone(content: EllipseContent(model: model), onHeight: { [weak self] height in self?.onHeight?(height) }))
    }
  ...
  public func reset() {
    ...
    model.active = false
    controller?.detach(); controller = nil; model = EllipseModel()
  }
}
private struct EllipseContent: View {
  @ObservedObject var model: EllipseModel
  var body: some View {
    Group {
