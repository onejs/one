...
@objcMembers public final class OneNativeCircleView: UIView, OneNativeComposable {
  private var model = CircleModel()
  public var onHeight: ((CGFloat) -> Void)?
  private var controller: OneNativeHostingController<OneNativeMeasuredStandalone<CircleContent>>?
  ...
  public func compositionContent() -> AnyView { AnyView(CircleContent(model: model)) }
  ...
  private func updateHost() {
    ...
    if controller == nil {
      bindCallbacks()
      controller = OneNativeHostingController(rootView: OneNativeMeasuredStandalone(content: CircleContent(model: model), onHeight: { [weak self] height in self?.onHeight?(height) }))
    }
  ...
  public func reset() {
    ...
    model.active = false
    controller?.detach(); controller = nil; model = CircleModel()
  }
}
private struct CircleContent: View {
