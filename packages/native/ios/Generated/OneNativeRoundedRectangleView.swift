...
@objcMembers public final class OneNativeRoundedRectangleView: UIView, OneNativeComposable {
  private var model = RoundedRectangleModel()
  public var onHeight: ((CGFloat) -> Void)?
  private var controller: OneNativeHostingController<OneNativeMeasuredStandalone<RoundedRectangleContent>>?
  ...
  public func configure(_ fill: UIColor?, stroke: UIColor?, lineWidth: Double, cornerRadius: Double) {
  ...
  public func compositionContent() -> AnyView { AnyView(RoundedRectangleContent(model: model)) }
  ...
  private func updateHost() {
    ...
    if controller == nil {
      bindCallbacks()
      controller = OneNativeHostingController(rootView: OneNativeMeasuredStandalone(content: RoundedRectangleContent(model: model), onHeight: { [weak self] height in self?.onHeight?(height) }))
    }
  ...
  public func reset() {
    ...
    model.active = false
    controller?.detach(); controller = nil; model = RoundedRectangleModel()
  }
}
private struct RoundedRectangleContent: View {
