...
private final class EditButtonModel: ObservableObject {

  @Published var accessibility = OneNativeAccessibility()
  @Published var swiftStyle = OneNativeStyle()
  var active = false
}
@objcMembers public final class OneNativeEditButtonView: UIView, OneNativeComposable {
  private var model = EditButtonModel()
  public var onHeight: ((CGFloat) -> Void)?
  private var controller: OneNativeHostingController<OneNativeMeasuredStandalone<EditButtonContent>>?
  ...
  public func compositionContent() -> AnyView { AnyView(EditButtonContent(model: model)) }
  ...
  private func updateHost() {
    ...
    if controller == nil {
      bindCallbacks()
      controller = OneNativeHostingController(rootView: OneNativeMeasuredStandalone(content: EditButtonContent(model: model), onHeight: { [weak self] height in self?.onHeight?(height) }))
    }
  ...
  public func reset() {
    ...
    model.active = false
    controller?.detach(); controller = nil; model = EditButtonModel()
  }
}
private struct EditButtonContent: View {
  @ObservedObject var model: EditButtonModel
  var body: some View {
    EditButton()
