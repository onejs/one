...
function checkIfRootIsPrerendering(root, renderLanes) {
  return (
    0 === (root.pendingLanes & ~(root.suspendedLanes & ~root.pingedLanes) & renderLanes)
  )
}
...
function initInput(
  ...
  if (null != value || null != defaultValue) {
    if (
      !(('submit' !== type && 'reset' !== type) || (void 0 !== value && null !== value))
    )
      return
...
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  if (
    ...
    if (didSuspend)
      ...
      (pushPrimaryTreeSuspenseHandler(workInProgress), '$!' === nextInstance.data)
    ) {
  ...
  if (null === resource.instance)
    switch (resource.type) {
      ...
      case 'stylesheet':
        ...
        instance$263 = (hoistableRoot.ownerDocument || hoistableRoot).createElement(
          'link'
        )
        markNodeAsHoistable(instance$263)
