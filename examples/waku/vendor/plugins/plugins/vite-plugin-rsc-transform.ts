...
export function rscTransformPlugin(
  ...
  return {
    ...
    async transform(code, id, options) {
      ...
      const resolve = async (specifier: string, { parentURL }: { parentURL: string }) => {
        if (!specifier) {
          return { url: '' }
        }
      ...
      const load = async (url: string) => {
        ...
        source = source!.replace(/^((?:import [^;]+;\s*)*)((?:"use [^"]+";\s*)*)/, '$2$1')
      ...
      if (opts.isBuild) {
        ...
        if (
          /^import {registerClientReference} from "react-server-dom-webpack\/server";/.test(source)
        ) {
          // HACK tweak registerClientReference for production
          source = source.replace(
            /registerClientReference\(function\(\) {throw new Error\("([^"]*)"\);},"[^"]*","([^"]*)"\);/gs,
            `registerClientReference(function() {return "$1";}, "${getClientId(id)}", "$2");`
          )
        }
        if (
          /;import {registerServerReference} from "react-server-dom-webpack\/server";/.test(source)
        ) {
