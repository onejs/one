...
export class Astro extends Component implements Link.Linkable {
  ...
  constructor(name: string, args: AstroArgs = {}, opts: ComponentResourceOptions = {}) {
    ...
    this.registerOutputs({
      _hint: all([this.cdn.domainUrl, this.cdn.url]).apply(([domainUrl, url]) => domainUrl ?? url),
      _metadata: {
        mode: 'deployed',
        path: sitePath,
    ...
    function loadBuildMetadata() {
      return outputPath.apply((outputPath) => {
        ...
        return JSON.parse(fs.readFileSync(filePath, 'utf-8')) as BuildMetaConfig
      })
    }
      ...
      return all([outputPath, buildMeta]).apply(([outputPath, buildMeta]) => {
        ...
        if (edge) {
          ...
          plan.behaviors.push(
            ...
            ...fs
              .readdirSync(path.join(outputPath, buildMeta.clientBuildOutputDir), {
                withFileTypes: true,
              })
              .map(
                (item) =>
  ...
  for (const [key, branch] of Object.entries(tree.branches)) {
    if (branch.nodes.length === 1) {
      ...
      if (node.type === 'endpoint') {
        ...
        flatTree.push([node.pattern, 2, node.redirectPath, node.redirectStatus])
      }
    } else {
