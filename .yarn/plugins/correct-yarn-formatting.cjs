...
class Logger {
  constructor(project) {
    this.logLevel = project.configuration.get("correctFormattingLogs") || "notice";
  }
  log(msg) {
  ...
  return {
    ...
    hooks: {
      validateProject(project) {
        ...
        origPkgs.push([topPkgJsonPath, readFileSync(topPkgJsonPath).toString()]);
      ...
      afterAllInstalled(project) {
        ...
        origPkgs.forEach(([path, origPkgStr]) => {
          ...
          if (
            ...
            JSON.stringify(JSON.parse(curPkgStr)) === JSON.stringify(JSON.parse(origPkgStr))
          ) {
